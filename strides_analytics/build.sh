...
scan-build --use-analyzer /usr/local/llvm/7.0.0/bin/clang g++ \
 ...
 -I. -I$HOME/include -I/opt/ncbi/gcc/7.3.0/include/c++/7.3.0/ \
 -I/opt/ncbi/gcc/7.3.0/include/c++/7.3.0/x86_64-redhat-linux-gnu/ \
 s3tojson.cpp -L$HOME/lib -lpthread -lcurl
...
afl-g++ --std=c++17 -g -march=native -Wall -Wextra -Wpedantic  -o s3tojson_fuzz s3tojson.cpp -I$HOME/include/ -L$HOME/lib  -lpthread  -lcurl
afl-g++ --std=c++17 -g -march=native -Wall -Wextra -Wpedantic  -o nginxtojson_fuzz nginxtojson.cpp -I$HOME/include -L$HOME/lib -lpthread -lcurl
afl-fuzz -i fuzz_s3 -o findings_s3 -- ./s3tojson_fuzz
afl-fuzz -i fuzz_nginx -o findings_nginx -- ./nginxtojson_fuzz

# Parallel fuzzing
cd "$TMP" || exit
afl-fuzz -M fuzzer00    -i $HOME/strides/fuzz_s3 -o findings_s3 -- $HOME/strides/s3tojson_fuzz
for x in $(seq 20); do
afl-fuzz -S "fuzzer0$x" -i $HOME/strides/fuzz_s3 -o findings_s3 -- $HOME/strides/s3tojson_fuzz > "fuzz.$x.log" 2>&1 &
done
