...
static void handle_aws( const Options & options )
{
    ...
    {
        OPParser p( cin, outputs );
        p . parse(); 
    }
    else
    {
        OPMultiThreadedParser p( stdin, outputs, 100000, options.numThreads );
        p . parse();
        std::cout << "num-feed-sleeps = " << p . num_feed_sleeps << endl;
        std::cout << "thread-sleeps = " << OPMultiThreadedParser :: thread_sleeps . load() << endl;        
    }
