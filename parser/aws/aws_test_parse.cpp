...
TEST_F( AWSTestFixture, ErrorRecovery )
{
    ...
    ASSERT_EQ( "{\"_line_nr\":1,\"_unparsed\":\"line1 blah\",\"bucket\":\"blah\",\"owner\":\"line1\"}\n",
                s_outputs.get_ugly() );
    ASSERT_FALSE( s_outputs.get_good().empty() );
}
...
TEST_F( AWSTestFixture, repeated_time_shifts_tls_vers )
{
    ...
    std::string res = try_to_parse_ugly( input );
    ASSERT_FALSE( res.empty() );
}
...
TEST_F( AWSTestFixture, extra_data )
{
    ...
    ASSERT_FALSE( res.empty() );
    ASSERT_EQ( "more tokens follow", extract_value( res, "_extra" ) );
}
...
TEST_F( AWSTestFixture, two_lines_with_and_without_accessions )
{
    ...
    ASSERT_FALSE( res.empty() );
    size_t eol = res.find('\n');
    ASSERT_NE( string::npos, eol );
    const string res1 = res.substr( 0, eol );
    ASSERT_FALSE( res1.empty() );
    ASSERT_EQ( "SRR123456", extract_value( res1, "accession" ) );
    ASSERT_EQ( "SRR123456", extract_value( res1, "filename" ) );
    ASSERT_EQ( "SRR123456.1", extract_value( res1, "key" ) );
    const string res2 = res.substr( eol+1, res.length() );
    ASSERT_EQ( "", extract_value( res2, "accession" ) );
    ASSERT_EQ( "", extract_value( res2, "filename" ) );
...
"384e684d1d83331f62ec686c0aae5c0513b488404eca7a6744bbeb7f5c6a7834 sra-ca-run-4 [29/Jun/2021:22:48:34 +0000] 34.226.216.239 arn:aws:sts::184059545989:assumed-role/sra-developer-instance-profile-role/i-07cffb826ea1814dd WC0X4C0EB5NQJQGB REST.GET.BUCKET - \"GET /?list-type=2&delimiter=%2F&prefix=SRR13431598%2FSRR13431598.2&encoding-type=url HTTP/1.1\" 200 - 324 - 25 24 \"-\" \"aws-cli/1.18.147 Python/2.7.18 Linux/4.14.231-173.361.amzn2.x86_64 botocore/1.18.6\" - Sp3lYc2YbhTGkIh700Tmw/4m3Bt7AawTZlRpEY5GoFSByNYqyHiOfJF5MgLTC/MhtmyV21E84+Y= SigV4 ECDHE-RSA-AES128-GCM-SHA256 AuthHeader sra-ca-run-4.s3.amazonaws.com TLSv1.2 - - region b c d"
    );
    ASSERT_FALSE( res.empty() );
    ASSERT_EQ( "b c d", extract_value( res, "_extra" ) );
}
