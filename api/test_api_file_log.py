...
class Test_file_logging( unittest.TestCase ) :
    ...
    def verify_line_json( self, line : str, d ) :
        j = json.loads( line.strip() )
        for k, v in d.items() :
            self.assertEqual( j[ k ], v, "for key:'{}'".format( k ) )

    def verify_line_protobuf( self, line : str, d ) :
        m = logmon_pb2.LogMessage()
        m.ParseFromString( bytes.fromhex( line.rstrip() ) )
        for k, v in d.items() :
            self.assertEqual( getattr(m, k), v, "for key:'{}'".format( k ) )

    verify_line = verify_line_protobuf
